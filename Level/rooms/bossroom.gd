...
@onready var win_area = $WinArea  # Add reference to the Area2D node
...
func _ready():
	...
	# Connect the area_entered signal for Area2D-to-Area2D detection
	if win_area:
		print("Win area found, connecting area_entered signal")
		win_area.area_entered.connect(_on_win_area_area_entered)
	else:
		print("ERROR: Win area not found!")

func _on_win_area_area_entered(area):
	print("Area entered win area: ", area.name)
	if area.is_in_group("player"):
		print("Player detected, changing to win scene")
		get_tree().change_scene_to_file("res://ui/game-win screen/game-win.tscn")
	else:
		print("Area is not in player group")
